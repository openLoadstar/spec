...
업무·처리의 **흐름**을 한 장의 그림으로 기술한다. 프로젝트 전체 흐름을 담는 것이 주 용도이고, 한 부분이 복잡해지면 같은 그림 안에서 **구역(subgraph)으로 묶는다**. 그 자체로 독립적인 흐름이면 별도 FLOW로 두고 링크한다.
...
| **subflow** | 다른 흐름으로 이어지는 단계 | `id[[라벨]]` | **FLOW** |
...
mermaid 자체 문법이므로 위 표에 없는 도형·방향(`flowchart TD` 등)도 그릴 수 있다. 다만 표에 있는 여섯 종류는 **의미가 고정**이므로, 다른 뜻으로 쓰지 않는다.

## 하위 흐름 — 같은 그림 안의 구역

한 흐름이 커지면 그 일부를 **같은 그림 안에서** 묶는다. mermaid `subgraph`를 그대로 쓴다.

```
    scan --> parse
    subgraph g1[요소 파싱]
        parse[파일명 분리] --> summary[SUMMARY 추출]
        summary --> conn[CONNECTIONS 파싱]
    end
    conn --> ok{공통 봉투 있음?}
```

- 구역 안의 노드는 **평범한 노드와 똑같다** — 별도 파일도, 별도 규칙도 없다.
- 구역 id는 노드 id와 같은 규칙(영숫자·밑줄)이고, 제목은 자유 텍스트다.
- **중첩하지 않는다.** 구역 안에 구역을 두면 경계와 표시 상태가 금세 복잡해진다.
- 도구는 구역을 **접어서** 상자 한 칸으로 보여줄 수 있다. 접힘은 보는 방식일 뿐이고 파일은 언제나 펼쳐진 형태다.

`[[ ]]` 도형이 가리키는 **다른 FLOW 파일**(아래 참조 규칙)과는 다른 것이다. 그쪽은 독립된 흐름으로 가는 링크이고, 이쪽은 한 흐름 안의 구역이다. 한 흐름을 여러 곳에서 재사용해야 하거나 그 자체로 독립적인 의미가 있으면 별도 FLOW로, 단지 한 그림이 길어졌을 뿐이면 구역으로 나눈다.

## 참조 규칙

- `REFERENCES`의 키는 `DIAGRAM`에 나오는 mermaid 노드 id다. 값은 대상 **파일명**이며, FORMAT 접두어로 대상 폴더를 판단한다(`02.ELEMENT_FORMAT.md §4`).
- 값의 FORMAT이 `FLOW`이면 그 노드는 **다른 흐름으로 가는 링크**다. 도구는 그 FLOW를 열어 보여준다.
...
- **FLOW끼리 서로를 참조하면 순환이 된다.** 검증 패스는 순환을 보고한다 — 금지하지는 않는다(되돌아가는 업무 흐름은 실재한다).
- 한 흐름이 지나치게 커지면 구역으로 묶거나 별도 FLOW로 쪼갠다. WP의 GOAL이 길어지면 하위 WP로 분해하라는 신호인 것과 같은 기준이다.
